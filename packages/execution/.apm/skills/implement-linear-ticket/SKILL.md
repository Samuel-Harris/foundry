...
This skill requires the Linear MCP server. It is a runtime prerequisite and is not installed by APM. If the Linear MCP server is not available, stop immediately with an actionable error: name the missing server, state that the Linear MCP server must be configured for the host, link to the MCP setup documentation, and do not proceed with any other step.
