...
Gather scope by asking the user a single question with multiple-choice options (use the host's structured-question capability if available, otherwise plain chat). First call `list_teams` and `list_projects` (Linear MCP) in parallel to populate options dynamically:
...
If the user's scope description is genuinely ambiguous (not just missing filter values), read and invoke the deep-interview skill.
...
Display a summary and ask for approval with a single multiple-choice question (use the host's structured-question capability if available, otherwise plain chat):
...
Use the Linear MCP server. Run calls in parallel where possible.
...
1. `list_initiatives` — `includeProjects: true`, `includeSubInitiatives: true`
2. `list_projects` — `includeMilestones: true`
...
1. Run `mkdir -p .foundry/masterplans` and `date '+%Y-%m-%d_%H-%M-%S'` to get a sortable timestamp (POSIX `date`; the intent is a lexicographically sortable timestamp, not shell portability).
2. Ask the user for a short description slug (2–4 words, kebab-case) or derive one from scope.
3. Save to `.foundry/masterplans/{timestamp}_{description}_masterplan.md`
...
> `.foundry/masterplans/{filename}`

1. Ask if the user wants to adjust any section.

## Refresh Mode

When the user asks to refresh an existing masterplan:

1. **Locate**: Use the path the user provided, or find the most recent `*_masterplan.md` in `.foundry/masterplans/`.
