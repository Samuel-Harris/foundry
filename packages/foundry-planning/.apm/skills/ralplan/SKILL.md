...
All three agents are installed by this package.

| Agent     | Dispatch name | Role                                                                                     |
| --------- | ------------- | ---------------------------------------------------------------------------------------- |
...
The Planner decides how to structure the plan, but must annotate **task dependencies** when the plan contains multiple tasks. This lets downstream execution (manual or via the `swarm` skill) determine ordering and parallelism.
...
The deep-interview skill runs a Socratic Q&A loop that scores ambiguity across weighted dimensions and refuses to exit until ambiguity drops below 20%. When it completes, it writes a crystal-clear spec to `.foundry/interviews/`. Use that spec as the task description for the planning loop.
...
- Create the plan file:
  - Run `mkdir -p .foundry/plans` then `date '+%Y-%m-%d_%H-%M-%S'` in the terminal to get a sortable timestamp (POSIX `date`; the intent is a lexicographically sortable timestamp, not shell portability).
  - Create `.foundry/plans/YYYY-MM-DD_HH-MM-SS_<feature-name>.md` and set `plan_path` to that path.
- Track iteration progress in a todo list

### 2. Invoke Planner

Dispatch the `planner` agent as a subagent (where the host cannot dispatch named agents, adopt the planner persona inline by reading its agent definition). The planner automatically skips its interview phase when context is pre-provided by an orchestrator.
...
- End with: "Execute directly. Never delegate to a subagent. Return the plan file path."
...
Dispatch the `architect` agent as a subagent (readonly). The architect gathers context via parallel reads and searches, performs deep analysis, and returns prioritised recommendations with `file:line` citations.
...
Dispatch the `critic` agent as a subagent (readonly). Provide the plan file path. The Critic evaluates four criteria (clarity, verifiability, context completeness, big picture), simulates implementation of 2–3 tasks, and returns an OKAY or REJECT verdict with specific improvements.
...
Update the todo list after each verdict.

## Handoff to Swarm

Once approved, a multi-task plan with dependency annotations can be handed to the `swarm` skill for execution. The swarm reads the dependency graph and file lists to determine batch grouping and agent selection — it skips its own analysis phase when given a ralplan. See the swarm skill for details.
...
8. **No delegation chains** — every agent prompt ends with "Execute directly. Never delegate to a subagent."
