...
3. Launch both subagents in the same message, in the background if the host supports it:
   - dispatch the `thermo-nuclear-review-subagent` agent for bugs, breakages, security, devex regressions, feature-flag leaks, and other branch-audit risks.
   - dispatch the `thermo-nuclear-code-quality-review-subagent` agent for maintainability, structure, file-size growth, spaghetti, abstractions, and codebase-health risks.
