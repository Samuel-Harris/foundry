...
2. **From a validated plan** — a plan file containing tasks, file lists, and a dependency graph is provided. The swarm skips its own analysis and planning and uses the plan directly.
...
```
        ...
        +------------+------------+
        |                         |
  User describes task     A validated plan is provided
        |                         |
        v                         v
  ...
  Phase 1.5: Short-circuit
        |                         |
        v                         v
  Phase 2: Plan             Phase 2: Plan from plan file
        |                         |
        +------------+------------+
                     |
                     v
           ...
           Dispatch batch as parallel subagents
                     |
           +---------+---------+---------+
           |         |         |         |
           v         v         v         v
         ...
         Result1  Result2  Result3  Result4
                     ...
                     v
           [COLLECT & ASSESS]
           - Track completed / failed in a todo list
...
> **Skip this phase when a validated plan file is provided.** Proceed directly to Phase 2 (plan-file variant).
...
1. Choose the most appropriate agent for each subtask individually — different subtasks can use different agent types and tiers within the same swarm:
   ...
   | `build-fixer-medium` | Multiple or complex build/type errors                                 |
   | `architect`          | Read-only analysis (readonly)                                         |

2. Determine batch grouping: agent count per batch = min(subtask_count_remaining, 4)
3. Create a todo list tracking all subtasks
...
For each agent in the batch, dispatch a subagent with:

- the chosen agent type and tier for this subtask (the tier determines the model — no need to override `model`)
- a prompt that includes specific file paths, clear instructions, and "Execute directly. Never delegate to a subagent."

Example dispatch (a single message prompting up to 4 parallel subagents):

```
Agent 1: { agent: "build-fixer-low", prompt: "Fix type error in backend/shared/models/user.py: ... Execute directly. Never delegate to a subagent." }
Agent 2: { agent: "executor-medium", prompt: "Add validation logic in backend/copilot/logic/auth.py: ... Execute directly. Never delegate to a subagent." }
Agent 3: { agent: "executor-low", prompt: "Add missing import in backend/copilot/api/endpoints/projects.py: ... Execute directly. Never delegate to a subagent." }
```
...
- **Success** → mark completed in the todo list
...
1. **Max 4 concurrent agents** — the host's concurrent subagent limit (default 4)
...
4. **No delegation chains** — every agent prompt must include "Execute directly. Never delegate to a subagent."
5. **Todo list for progress** — track all subtasks with status updates
...
### From a validated plan

```
→ A validated plan is provided at .foundry/plans/add_user_feature.md with 5 tasks:
