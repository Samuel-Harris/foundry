...
2. **Detect brownfield vs greenfield**:
   - Dispatch the `explore` agent as a subagent to check if cwd has existing source code, package files, or git history
   - If source files exist AND the user's idea references modifying/extending something: **brownfield**
   - Otherwise: **greenfield**
3. **For brownfield**: Use the `explore` agent to map relevant codebase areas, store as `codebase_context`
4. **Track interview progress** in a todo list (rounds, current phase)
...
Ask the user a single question with contextually relevant options plus a free-text option (use the host's structured-question capability if available, otherwise plain chat):
...
Track which modes have been used in the todo list to prevent repetition.
...
2. Create the output directory with `mkdir -p .foundry/interviews`, then **run `date '+%Y-%m-%d_%H-%M-%S'` in the terminal** to get a sortable timestamp (POSIX `date`; the intent is a lexicographically sortable timestamp, not shell portability)
3. Write to `.foundry/interviews/{timestamp}_{slug}_interview.md` (e.g., `2026-03-04_14-22-10_auth-refactor_interview.md`)
...
{brownfield: relevant codebase findings from the explore agent}
