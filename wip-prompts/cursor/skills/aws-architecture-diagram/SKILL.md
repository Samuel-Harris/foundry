...
description: Generate AWS architecture diagrams as Mermaid flowcharts with nested account/VPC subgraphs, numbered data-flow steps, and same-account placement. Invoke with /aws-architecture-diagram to analyze Terraform, CDK, or CloudFormation, or to brainstorm a stack. Writes a markdown file with a mermaid fence. Do not produce draw.io, diagrams.net, or .drawio files.
disable-model-invocation: true
---

# AWS Architecture Diagram

Generate AWS architecture diagrams as **Mermaid** `flowchart TB` diagrams. Do not write draw.io XML, `.drawio` files, or diagrams.net preview URLs. Do not call or require the Cursor AWS plugin.
...
- Output directory: if the repo has `.devtools/agents/artefacts/`, write to `.devtools/agents/artefacts/architecture-diagrams/<descriptive-name>.md`; otherwise write to `.cursor/artefacts/architecture-diagrams/<descriptive-name>.md`. Create the directory. Do not commit the diagram unless the user asks.
...
3. If there is no AWS infrastructure, scan Dockerfiles, databases, APIs, ML stacks, and brokers and still draw them as labeled nodes inside or outside `AWS Cloud` as appropriate.
4. Confirm only when the target system is still ambiguous.
5. Ask which diagram type fits, unless the type is obvious (VPC/container for an isolated ECS plane).
...
Optional `classDef` (keep it light; Cursor/GitHub mermaid supports it):
...
1. Write `OUT_DIR/<descriptive-name>.md` with the title, subtitle, mermaid fence, and (if the legend is on) the same numbered steps as markdown below the fence so they remain readable if mermaid fails to render.
2. Do not write `.drawio`. Do not run `scripts/` even if that folder exists.
...
- Do not install Cursor plugins, do not terraform apply, do not commit gitignored artefacts, do not emit draw.io or diagrams.net URLs
