...
Same-account: in-account APIs, batch producers, and other same-account compute are not Users actors. Draw them as ECS/Fargate/EC2 inside the product VPC, still inside AWS Cloud. `#f5f5f5` Users boxes are only for humans, on-premises, or third-party SaaS — the post-processor will shove those boxes out of AWS Cloud.

**External actor coordinates**: True external actors MUST have coordinates that place them visually OUTSIDE the AWS Cloud group rectangle — at least 40px from the boundary.
