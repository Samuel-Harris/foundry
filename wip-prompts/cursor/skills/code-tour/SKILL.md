...
the code in Cursor. Tell one logically ordered story in chat, cite the changed code in the
worktree, explain consequential commit and review history, and account for non-behavioral churn in
an appendix.
...
- Return the tour in chat. Never write a report, story, visualization, or coverage-ledger file.
...
- Honor an explicit PR, base/head comparison, staged, unstaged, or all-local scope instead.
...
For a PR, use its actual `baseRefName`, not an assumed `dev` base. For a non-PR branch comparison
without a stated base, use the associated PR base when available; otherwise ask one focused
question offering `origin/dev` for normal development and `origin/main` for a hotfix.
...
- The final contents of every behavior-affecting file.
...
- Classification: `behavioral`, `meaningful-test`, or `non-behavioral`.
- Logical story group.
...
For a large changelist, launch up to four parallel read-only `explore` subagents over disjoint
logical domains. Prefer product/domain seams over arbitrary directory slices. Give each agent:
...
- A requirement to return behavior, motivation, connections, meaningful tests, final-code line
  ranges, non-behavioral appendix entries, and unresolved evidence gaps.
...
- A mechanical rename or relocation with behavior preserved.
- Mechanical fixture, snapshot, or test maintenance that does not add behavioral evidence.

If classification is uncertain, keep the hunk in the main tour. Never hide a behavior-affecting
change in the appendix merely because it is small or appears in a test, workflow, or config file.

Discuss meaningful tests beside the behavior they prove. Explain what regression each test would
...
3. Runtime/data flow.
4. Integration, deployment, and operational behavior.
5. Meaningful verification.

Adapt those chapters to the actual change. Cross-domain files serving one behavior belong in the
same chapter.
...
Every behavioral group must contain clickable Cursor repository citations with actual code:
...
- Cite every behavior-affecting hunk or a final range that clearly contains it; representative
  examples alone are not enough when other behavior would go unlinked.
...
- Use multiple citations when a behavior crosses contracts, implementation, callers, and tests.
...
- Name the deleted path and behavior explicitly.
...
<Focused Cursor code citations>

<Meaningful tests beside the behavior they prove>

### Appendix: non-behavioral changes
...
- Lead each chapter with the behavioral claim, then show the code that makes it true.
- Use plain language and define unfamiliar mechanisms briefly.
- Mention every behavior-affecting change in the main tour.
...
3. Every behavioral hunk is explained in the main tour.
4. Every meaningful test is beside the behavior it proves.
5. Every non-behavioral path is named in the appendix.
...
Test with `/code-tour` on the checked-out branch for PatentCopilot PR #22766.

Pass only when:

- The current branch's PR is resolved without asking for a base.
- The response is chat-only and creates no `pr-story-22766.md` or other report.
- The main tour covers centralized us-dev compute, queues/storage/IAM/networking, callback routing,
  checksum replay, terminal redrive and DLQ relay, shared image deployment, temporary
  opt-in/removal, and monitoring.
- It explains the shared GPU refactor, removed canary, fixed deployment blockers, fixed temp
  opt-in, and explicitly accepted origin-spoofing risk as evolution/context.
- Every behavioral group has clickable Cursor citations to final code.
- Meaningful tests appear beside the behavior they prove.
- Every remaining path is named in the non-behavioral appendix.
- The final deep-dive choices are specific to this PR.

Fail when the response only summarizes the final diff, silently omits behavior because the PR is
large, uses plain paths or `story-diff` instead of Cursor citations, performs a new review, or
