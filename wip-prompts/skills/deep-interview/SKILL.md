...
- Gather codebase facts via `explore` subagent BEFORE asking the user about them
...
2. **Detect brownfield vs greenfield**:
   - Use a Task with `subagent_type: "explore"` and `model: "fast"`: check if cwd has existing source code, package files, or git history
